...
export const editPlayer = async (playerCnic: string, updates: Partial<IPlayer>) => {
  const player = await findPlayerByCnic(playerCnic);

  if (Object.keys(updates).length === 0) {
    throw new BadRequestError('No valid fields provided for update');
  }

  // ✅ Check if bib_number is being updated and ensure it's unique
  if (updates.bib_number !== undefined && updates.bib_number !== null) {
    const existingPlayer = await Player.findOne({ bib_number: updates.bib_number });

    if (existingPlayer && existingPlayer._id.toString() !== player._id.toString()) {
      throw new BadRequestError(`Bib number ${updates.bib_number} is already assigned to another player.`);
    }
  }
